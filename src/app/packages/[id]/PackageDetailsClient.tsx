...
export default function PackageDetailsClient({ 
  ...
  const handleReviewSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await addReview({ packageId: pkg._id, rating, comment });
      if (res.success) {
        setComment('');
        router.refresh();
      } else {
  ...
  const handleChatShortcut = () => {
    router.push(`/chat?packageId=${pkg._id}&title=${encodeURIComponent(pkg.title)}`);
  };
    ...
    <div className="bg-[var(--color-neutral-bg)] min-h-screen pb-24">
      ...
      <div className="container mx-auto px-4 md:px-8 pt-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 h-[400px] md:h-[500px]">
          ...
          <div className="hidden md:flex flex-col gap-4">
            {pkg.images.slice(1, 3).map((img: string, idx: number) => (
              <div 
                key={idx} 
      ...
      <div className="container mx-auto px-4 md:px-8 mt-8 flex flex-col lg:flex-row gap-8 relative">
        ...
        <div className="flex-1">
          ...
          <div className="mb-12">
            <h2 className="text-2xl font-heading font-bold text-[var(--color-text-main)] mb-6">Reviews ({initialReviews.length})</h2>
            ...
            <div className="space-y-4">
              ...
              {initialReviews.length === 0 && <p className="text-gray-500">No reviews yet.</p>}
            </div>
          </div>
        </div>
